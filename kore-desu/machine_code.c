...
int MachineFromAssembly(const char *asm_file, const char *out_file) {
    ...
    while(fgets(line, sizeof(line), in)) {
        ...
        // dmult
        else if(sscanf(line,"dmult %7[^,], %7s", r1,r2) == 2) {
            int rs = RegisterNumber(r1), rt = RegisterNumber(r2);
            if(rs >= 0 && rt >= 0) {
                code = Encode_R_Type(rs, rt, 0, 0, FUNCT_DMULT); 
                matched = 1; 
            }
        }
        // ddiv
        else if(sscanf(line,"ddiv %7[^,], %7s", r1,r2) == 2) {
            int rs = RegisterNumber(r1), rt = RegisterNumber(r2);
            if(rs >= 0 && rt >= 0) { 
                code = Encode_R_Type(rs, rt, 0, 0, FUNCT_DDIV); 
                matched = 1; 
            }
        }
        // mflo
        else if(sscanf(line,"mflo %7s", r1) == 1) {
            int rd = RegisterNumber(r1);
            if(rd >= 0) {
        ...
        // mfhi
        else if(sscanf(line,"mfhi %7s", r1) == 1) {
            int rd = RegisterNumber(r1);
            if(rd >= 0 ){ 
        ...
        // ld (load doubleword)
        else if(sscanf(line,"ld %7[^,], %7[^)]", r1,r2) == 2) {
            int rt = RegisterNumber(r1), rs = 0;
            int16_t imm = 0;
            char var_name[64] = {0};
            sscanf(r2, "%63[^ (]", var_name);
        ...
        // sd (store doubleword)
        else if(sscanf(line,"sd %7[^,], %7[^)]", r1,r2) == 2) {
            int rt = RegisterNumber(r1), rs = 0;
            int16_t imm = 0;
            char var_name[64] = {0};
            sscanf(r2, "%63[^ (]", var_name);
